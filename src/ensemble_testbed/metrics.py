"""
Tools to calculate performance metrics given predictions and labels
"""
import numpy as np
...
class VarianceData(object):
    ...
    def register(self, preds, labels, modelname):
        ...
        """
        assert modelname.startswith(self.modelprefix)
        "modelname must start with modelprefix"
        for model, modeldata in self.models.items():
            assert np.all(labels == modeldata["labels"])
            "labels must match already registered."
            assert np.allclose(np.sum(preds, axis=1), 1)
